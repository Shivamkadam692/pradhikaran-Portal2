...
export default function AllReceivedAnswers() {
  ...
  const setAnswerStatus = async (answerId, status, remark = '') => {
    try {
      ...
      // Reload the current question's answers to reflect the status change
      await loadAnswers(selectedQuestion._id);
    } catch (e) {
      setError(e.response?.data?.message || 'Failed to update answer status');
    }
  };
  ...
  return (
    <div className="view-all-answers">
      ...
      <h1 className="page-title">All Received Answers</h1>
      ...
      {questions.length === 0 ? (
        ...
        <div className="row">
            ...
            {selectedQuestion && (
              <div className="glass p-3">
                ...
                {selectedQuestion.deadline && (
                  <div className="mb-2">
                    <strong>Deadline:</strong> {formatDate(selectedQuestion.deadline)}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="col-md-8">
            <div className="glass p-3">
              <h3>Received Answers ({answers.length})</h3>
              
              {answers.length === 0 ? (
                <div>No answers received for this question yet.</div>
              ) : (
                <div className="answers-list">
                  {answers.map((answer) => (
                    <div key={answer._id} className="answer-block mb-3">
                      ...
                      {answer.attachments && answer.attachments.length > 0 && (
                        <div className="attachments-section mt-3">
                          ...
                          <div className="attachments-list">
                            {answer.attachments.map((attachment, idx) => (
                      ...
                      {/* Status update buttons for pending answers */}
                      {selectedQuestion?.status !== 'finalized' && 
                       selectedQuestion?.status !== 'locked' && 
                       answer.status === 'pending_review' && (
                        <div className="answer-actions mt-3">
                          <button
                            type="button"
                            className="btn btn-sm btn-success me-2"
                            onClick={() => setAnswerStatus(answer._id, 'accepted')}
                          >
                            Accept
                          </button>
                          <button
                            ...
                            onClick={() => setAnswerStatus(answer._id, 'rejected')}
                          >
                            Reject
                          </button>
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (remark !== null) {
                                setAnswerStatus(answer._id, 'update_requested', remark);
                              }
                            }}
                          >
                            Request Update
